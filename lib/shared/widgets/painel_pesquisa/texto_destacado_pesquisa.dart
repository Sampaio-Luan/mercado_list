...
class TextoDestacadoPesquisa extends StatelessWidget {
  const TextoDestacadoPesquisa({
    super.key,
    required this.texto,
  ...
  List<_IntervaloDestaque> _localizarIntervalosCoincidentes() {
    ...
    final termoNormalizado = SimilaridadeTexto.normalizar(textoPesquisa);
    if (termoNormalizado.isEmpty) return intervalos;

    final textoNormalizado = SimilaridadeTexto.normalizar(texto);
